...
export type MergeContractCallTxData<
  C extends CompactContract.Any = CompactContract.Any,
  PCK extends CompactContract.ProvableCircuitId<C> = CompactContract.ProvableCircuitId<C>,
> = {
...
function getCallIntentSegment(callTx: MergeContractCallTxData): number | undefined {
  return Array.from(callTx.callTxData.private.unprovenTx.intents?.entries() ?? []).find(
...
>(
  ...
  callTxs: [MergeContractCallTxData<C, PCK>, MergeContractCallTxData<D, Q>],
): Offer | undefined {
  if (!offer) {
    return offer;
  }
  ...
  callTxs.forEach((callTx) => {
    contractOwnedOutputCoinsByCommitment(callTx.zswapLocalState).forEach((coin, commitment) => {
  ...
  const duplicatedCommitments = new Set(
    Array.from(outputsByCommitment.entries())
      .filter(([, outputs]) => outputs.length > 1 && knownContractOwnedOutputs.has(outputs[0].commitment))
...
>(
  ...
  current: MergeContractCallTxData<C, PCK>,
  next: MergeContractCallTxData<D, Q>,
): void {
  const targetSegment = getCallIntentSegment(current);
  const sourceSegment = getCallIntentSegment(next);

  if (targetSegment === undefined || sourceSegment === undefined || targetSegment === sourceSegment || !mergedTx.intents) {
    return;
  }
...
>(
  ...
  const mergedTx = current.callTxData.private.unprovenTx.merge(next.callTxData.private.unprovenTx);
