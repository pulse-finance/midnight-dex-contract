...
describe("MarketOrder", () => {
  ...
  it("opens, reserves, sends, receives, and closes", () => {
    ...
    expect(calls.map((call) => call[2])).toContain(
      Buffer.from(ammFundOrderXCircuitHash).toString("hex"),
    )
    ...
    expect(
      closed.context.currentQueryContext.effects.claimedContractCalls.map((call) => call[2]),
  ...
  it("requires spam return coins to be merged before close", () => {
    ...
    expect(() => simulator.receiveFromAmm({ amount: 1n, coinIndex: 1n })).toThrow(
