...
export const mintLpOtherUser = {
  bytes: encodeCoinPublicKey(mintLpOtherUserPublicKey),
}
export const encodedMintLpContractAddress = {
  bytes: encodeContractAddress(mintLpContractAddress),
}
export const encodedMintLpAmmContractAddress = {
  bytes: encodeContractAddress(mintLpAmmContractAddress),
}
...
export class MintLpOrderSimulator {
  ...
  sendToAmm({
    ...
    calleeRnd = mintLpCallOpening,
    ammTick = 0n,
    ammTickRnd = mintLpTickCallOpening,
  } = {}) {
    let result = this.contract.circuits.MintLpOrderReserveAmmSlot(
      this.makeContext(sender),
      ammTick + 1n,
      ammTickRnd,
    )
  ...
  close({
    sender = mintLpOwner,
    secret = mintLpOwnerSecret,
    ammTick = 1n,
    ammTickRnd = mintLpTickCallOpening,
  } = {}) {
    const contract =
      secret === mintLpOwnerSecret ? this.contract : MintLpOrderSimulator.makeContract(secret)
