...
const COMPACT_VERSION = "0.5.1";
const INSTALLER_URL = `https://github.com/midnightntwrk/compact/releases/download/compact-v${COMPACT_VERSION}/compact-installer.sh`;
...
function installationRoot() {
  ...
  return path.join(base, `compact-v${COMPACT_VERSION}`, PLATFORM_ARCH);
}
...
export async function ensureCompactBinary() {
  ...
  console.error(
    `[dex-contract] Installing Compact ${COMPACT_VERSION} for ${PLATFORM_ARCH} into ${installDir}`,
  );
